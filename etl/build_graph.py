...
# Pesos del Economic Power Index (suman 100)
EPI_W = {
    "ingresos": 35,
    "empleados": 15,
    "empresas": 10,
...
def main() -> None:
    ...
    for gid, mids in members.items():
        ingresos = sum(empresas[m]["ingresos_aprox"] or 0 for m in mids)
        empleados = sum(empresas[m]["empleados_aprox"] or 0 for m in mids)
    ...
    norm = {k: minmax({g: raw[g][k] for g in raw}, log=k in ("ingresos", "empleados"))
            for k in EPI_W}
    ranking = []
    for gid in raw:
        ...
        ranking.append({
            ...
            **{k: raw[gid][k] for k in ("ingresos", "empleados", "empresas", "sectores")},
